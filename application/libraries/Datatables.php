  ...
  */
  class Datatables
  {
    ...
    */
    ...
    /**
    * instead of the database name and then connecting
    * this setter takes a pointer to a ready-to-go db object
    */
    public function set_DB($theDB)
    {
      $this->zeDB = $theDB;
    }
    ...
    */
    public function select($columns, $backtick_protect = TRUE)
    {
      ...
      $this->zeDB->select($columns, $backtick_protect);
      return $this;
    }
    ...
    */
    public function where($key_condition, $val = NULL, $backtick_protect = TRUE)
    {
      ...
      $this->zeDB->where($key_condition, $val, $backtick_protect);
      return $this;
    }
    ...
    */
    public function or_where($key_condition, $val = NULL, $backtick_protect = TRUE)
    {
      ...
      $this->zeDB->or_where($key_condition, $val, $backtick_protect);
      return $this;
    }
    ...
    */
    public function like($key_condition, $val = NULL, $backtick_protect = TRUE)
    {
      ...
      $this->zeDB->like($key_condition, $val, $backtick_protect);
      return $this;
    }
    ...
    * @return mixed
    */
    public function not_like($key_condition, $val = NULL)
    {
      $this->not_like[] = array($key_condition, $val);
      $this->zeDB->not_like($key_condition, $val);
      return $this;
    }
    ...
    */
    private function get_paging()
    {
      ...
      if($iLength != '' && $iLength != '-1')
        $this->zeDB->limit($iLength, ($iStart)? $iStart : 0);
    }
    ...
    */
    private function get_ordering()
    {
      ...
      for($i = 0; $i < intval($this->ci->input->post('iSortingCols')); $i++) {
        ...
        if(isset($mColArray[intval($this->ci->input->post('iSortCol_' . $i))-1]) && in_array($mColArray[intval($this->ci->input->post('iSortCol_' . $i))-1], $columns) && $this->ci->input->post('bSortable_'.intval($this->ci->input->post('iSortCol_' . $i))) == 'true') {
          	
          	if( intval( $this->ci->input->post('iSortCol_' . $i) )-1 >= 0 ) {
          	
          		$this->zeDB->order_by($mColArray[intval( $this->ci->input->post('iSortCol_' . $i) )-1], $this->ci->input->post('sSortDir_' . $i));
          	
          	} else {
          	
          		$this->zeDB->order_by($mColArray[intval( $this->ci->input->post('iSortCol_' . $i) )], $this->ci->input->post('sSortDir_' . $i));
    ...
    */
    private function get_filtering()
    {
      ...
      $sWhere = '';
      $sSearch = $this->zeDB->escape_like_str($this->ci->input->post('sSearch'));
      $mColArray = array_values(array_diff($mColArray, $this->unset_columns));
      $columns = array_values(array_diff($this->columns, $this->unset_columns));
      ...
      if($sSearch != '')
      	for($i = 0; $i < count($mColArray); $i++)
        	if($this->ci->input->post('bSearchable_' . $i) == 'true' && in_array($mColArray[$i], $columns)) {
				...
				if (strpos($this->select[$mColArray[$i]],".") !== false) {
				    
					$ss = str_replace(".", "`.`", $this->select[$mColArray[$i]]);
					
				} else {
					
					$ss = $this->select[$mColArray[$i]];
					
				}
				
				$sWhere .= "`".$ss."`" . " LIKE '%" . $sSearch . "%' OR ";
			
			}
      $sWhere = substr_replace($sWhere, '', -3);

      if($sWhere != '')
        $this->zeDB->where('(' . $sWhere . ')');
        ...
        if(isset($_POST['sSearch_' . $i]) && $this->ci->input->post('sSearch_' . $i) != '' && in_array($mColArray[$i], $columns))
        {
          ...
          foreach($miSearch as $val)
          {
            if(preg_match("/(<=|>=|=|<|>)(\s*)(.+)/i", trim($val), $matches))
              $this->zeDB->where($this->select[$mColArray[$i]].' '.$matches[1], $matches[3], true);
            elseif(!empty($sRangeSeparator) && preg_match("/(.*)$sRangeSeparator(.*)/i", trim($val), $matches))
            {
              ...
              if(!empty($matches[1]) || !empty($matches[2]))
                $this->zeDB->where($rangeQuery);
            }
            else
              $this->zeDB->where($this->select[$mColArray[$i]] . ' LIKE', '%' . $val . '%');
          }
        }
      }

      foreach($this->filter as $val)
        $this->zeDB->where($val[0], $val[1], $val[2]);
    }
    ...
    */
    ...
    {
      return $this->zeDB->get($this->table);
    }
    ...
    */
    private function produce_output($output, $charset)
    {
      ...
      foreach($rResult->result_array() as $row_key => $row_val)
      {
        ...
        foreach($this->add_columns as $field => $val)
          if($this->check_mDataprop()) {
            ...
            //$aaData[$row_key][0] = $this->exec_replace($val, $aaData[$row_key]);
            array_unshift($aaData[$row_key], $this->exec_replace($val, $aaData[$row_key]));
            }
      ...
      $sColumns = array_merge_recursive(array_keys($this->add_columns), $sColumns);
    ...
    */
    private function get_total_results($filtering = FALSE)
    {
      ...
      foreach($this->joins as $val)
        $this->zeDB->join($val[0], $val[1], $val[2]);

      foreach($this->where as $val)
        $this->zeDB->where($val[0], $val[1], $val[2]);

      foreach($this->or_where as $val)
        $this->zeDB->or_where($val[0], $val[1], $val[2]);

      foreach($this->group_by as $val)
        $this->zeDB->group_by($val);

      foreach($this->like as $val)
        $this->zeDB->like($val[0], $val[1], $val[2]);

      if(strlen($this->distinct) > 0)
      {
        $this->zeDB->distinct($this->distinct);
        $this->zeDB->select($this->columns);
      }

      $query = $this->zeDB->get($this->table, NULL, NULL, FALSE);
